...
library StringTestUtility {
    function startsWith(string memory ref, string memory test) internal pure returns (bool) {
    ...
    function endsWith(string memory ref, string memory test) internal pure returns (bool) {
        bytes memory refBytes = bytes(ref);
        bytes memory testBytes = bytes(test);
        if (testBytes.length > refBytes.length) {
            return false;
        ...
        for (uint256 i = 0; i < testBytes.length; ++i) {
            if (refBytes[refBytes.length - 1 - i] != testBytes[testBytes.length - 1 - i]) {
                return false;
            }
        }
        return true;
    ...
    function contains(string memory str, string memory test) internal pure returns (bool) {
        bytes memory strBytes = bytes(str);
        bytes memory testBytes = bytes(test);
    ...
    function splice(string memory str, uint256 start, uint256 end) internal pure returns (string memory) {
        bytes memory strBytes = bytes(str);
    ...
    function equals(string memory ref, string memory test) internal pure returns (bool) {
